...
void Spinbox::handle(genv::event ev, Widget* w) {
    if (n > lowerlim && n < upperlim) {
        ...
        if (btn1->on_widget(ev) && ev.button > 0) {
            n++;
        } else if (btn2->on_widget(ev) && ev.button > 0) {
            n--;
        }
    ...
    } else if (n == lowerlim && (ev.keycode == key_up || (btn1->on_widget(ev) && ev.button > 0)))
        n++;
    else if (n == upperlim && (ev.keycode == key_down || (btn2->on_widget(ev) && ev.button > 0)))
        n--;
